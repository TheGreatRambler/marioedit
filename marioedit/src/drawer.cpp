...
namespace MarioEdit {
	namespace Level {
		...
		void Drawer::Setup() {
			...
			static uint8_t GS[] = {  };
		...
		void Drawer::SetGraphics(SkCanvas* canvas) {
			c = canvas;
		}

		sk_sp<SkImage> Drawer::ImageFromPath(std::string path) {
		...
		void Drawer::SetIsOverworld(bool isOverworld) {
			NowIO = isOverworld;
		}

		void Drawer::SetLog(bool log) {
			doLogging = log;
		}

		void Drawer::SetAssetFolder(std::string folder) {
			assetFolder = folder;
		}

		void Drawer::SetOffsetX(int x) {
			offsetX = x;
		}

		void Drawer::SetOffsetY(int y) {
			offsetY = y;
		}
		...
		void Drawer::DrawGridlines() {
			...
			c->drawRect(SkRect::MakeXYWH(0 + offsetX,
							(H - level.MapHdr.LiqEHeight - 0.5) * Zm + offsetY, W * Zm, H * Zm),
				level.MapHdr.Theme == 2 ? lavaPaint : waterPaint);
			c->drawRect(SkRect::MakeXYWH(0 + offsetX,
							(H - level.MapHdr.LiqSHeight - 0.5) * Zm + offsetY, W * Zm, H * Zm),
				level.MapHdr.Theme == 2 ? lavaPaint : waterPaint);
		}

		sk_sp<SkImage> Drawer::GetCachedTile(int x, int y, int w, int h) {
			int index = (y * 16 + x) * 3;
		...
		sk_sp<SkImage> Drawer::GetCachedPattern(int id) {
		...
		void Drawer::DrawTile(int tileX, int tileY, int tileW, int tileH, int x, int y,
		...
		void Drawer::DrawImage(uint32_t id, int x, int y, int targetWidth, int targetHeight) {
			if(doLogging) {
			...
			if(pattern) {
				const auto sprite = Data::ObjectLocation.at(id);
				c->drawImageRect(pattern,
					SkRect::MakeXYWH(0, 0, pattern->width(), pattern->height()),
					SkRect::MakeXYWH(x + offsetX, y + offsetY, targetWidth, targetHeight),
					SkSamplingOptions(SkFilterMode::kNearest), nullptr,
					SkCanvas::kStrict_SrcRectConstraint);
			}
		...
		void Drawer::DrawImageRotateOpacity(uint32_t id, double angle, double opacity, int x, int y,
			int targetWidth, int targetHeight) {
			if(doLogging) {
				fmt::print("Printing {} at X:{} Y:{} with W:{} and H:{}\n", id, x, y, targetWidth,
					targetHeight);
		...
		void Drawer::DrawCrp(unsigned char EX, int X, int Y) {
			...
			for(i = 0; i < level.MapCrp[EX - 1].NodeCount; i++) {
				DrawImage(Data::OBJ_CMN_SS, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);

				uint32_t path = 0;
				switch(level.MapCrp[EX - 1].Node[i]) {
				case 1: // L
					path = Data::OBJ_CMN_SL;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					XX -= 2;
					break;
				case 2: // R
					path = Data::OBJ_CMN_SR;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					XX += 2;
					break;
				case 3: // D
					path = Data::OBJ_CMN_SD;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					YY -= 2;
					break;
				case 4: // U
					path = Data::OBJ_CMN_SU;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					YY += 2;
					break;
				case 5: // LD
					path = Data::OBJ_CMN_SRD;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					YY -= 2;
					break;
				case 6: // DL
					path = Data::OBJ_CMN_SUL;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					XX -= 2;
					break;
				case 7: // LU
					path = Data::OBJ_CMN_SRU;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					YY += 2;
					break;
				case 8: // UL
					path = Data::OBJ_CMN_SDL;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					XX -= 2;
					break;
				case 9: // RD
					path = Data::OBJ_CMN_SLD;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					YY -= 2;
					break;
				case 10: // DR
					path = Data::OBJ_CMN_SUR;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					XX += 2;
					break;
				case 11: // RU
					path = Data::OBJ_CMN_SLU;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					YY += 2;
					break;
				case 12: // UR
					path = Data::OBJ_CMN_SDR;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					XX += 2;
					break;
				case 13: // RE
					path = Data::OBJ_CMN_SE;
					DrawImage(path, XX * Zm, (H - YY) * Zm, Zm * 2, Zm * 2);
					break;
				case 14: // LE
					path = Data::OBJ_CMN_SE;
		...
		void Drawer::DrawSnake(unsigned char EX, int X, int Y, int SW, int SH) {
			int XX = 0;
			int YY = 0;
			...
			for(i = 0; i < level.MapSnk[EX - 1].NodeCount; i++) {
				DrawImage(Data::OBJ_CMN_SS, XX * Zm, (H - YY) * Zm, Zm, Zm);
		...
		void Drawer::DrawMoveBlock(unsigned char ID, unsigned char EX, int X, int Y) {
			int XX = 0;
			int YY = 0;
			...
			for(m = 0; m <= 2; m++) {
				for(i = 0; i <= W; i++) {
					for(j = 0; j <= H; j++) {
						...
						case 2:
						case 16: // UL
							if(level.GroundNode[i + 2][j + 1] > 0
								|| level.GroundNode[i + 1][j] > 0) {
						...
						case 18: // DL
							if(level.GroundNode[i + 2][j + 1] > 0
								|| level.GroundNode[i + 1][j + 2] > 0) {
								level.GroundNode[i + 1][j + 1] = 1;
							}
							break;
						case 5:
						case 19: // DR
							if(level.GroundNode[i][j + 1] > 0
								|| level.GroundNode[i + 1][j + 2] > 0) {
								level.GroundNode[i + 1][j + 1] = 1;
							}
							break;
						case 12:
							if(level.GroundNode[i + 1][j] > 0
								|| level.GroundNode[i + 2][j + 1] == 1) {
								level.GroundNode[i + 1][j + 1] = 1;
							}
							break;
						case 13:
			...
			if(NowIO) {
				...
				case 12621: // 1
					if(level.MapHdr.Theme == 2) {
						DrawImage(level.LH.GameStyle | Data::OBJ_27A,
						...
						DrawImage(level.LH.GameStyle | Data::OBJ_27,
							(float)((level.LH.GoalX / 10.0 - 0.5) * Zm),
				...
				case 13133: // 3
					if(level.MapHdr.Theme == 2) {
						...
						DrawImage(level.LH.GameStyle | Data::OBJ_27,
							(float)((level.LH.GoalX / 10.0 - 0.5) * Zm),
							(level.MapHdr.BorT / 16 - level.LH.GoalY - 5 - 0.5) * Zm, Zm * 2,
							Zm * 2);
					}
					break;
				case 22349: // W
					if(level.MapHdr.Theme == 2) {
						...
						//"\img\" & LH.GameStyle.ToString & "\obj\27E ), CSng((LH.GoalX / 10 + 1.5)
						//* Zm), (MapHdr.BorT \ 16
						//- LH.GoalY - i) * Zm, Zm, Zm) Next G. DrawImage(level.LH.GameStyle
						//|Image.FromFile(PT & "\img\" &
						// LH.GameStyle.ToString &
						// "\obj\27B ), CSng((LH.GoalX / 10 - 0.5) * Zm), (MapHdr.BorT \ 16 -
						// LH.GoalY - 9)
						// * Zm, Zm, Zm) G. DrawImage(level.LH.GameStyle |Image.FromFile(PT &
						// "\img\" & LH.GameStyle.ToString & "\obj\27D ), CSng((LH.GoalX / 10 + 1.5)
						// * Zm), (MapHdr.BorT \ 16 - LH.GoalY - 9) * Zm, Zm, Zm)

						DrawImage(level.LH.GameStyle | Data::OBJ_27F,
							(float)((level.LH.GoalX / 10.0 - 0.5) * Zm),
							(float)(level.MapHdr.BorT / 16 - level.LH.GoalY - 8.5) * Zm, Zm,
							Zm * 9);
						DrawImage(level.LH.GameStyle | Data::OBJ_27,
							(float)((level.LH.GoalX / 10.0) * Zm),
							(float)(level.MapHdr.BorT / 16 - level.LH.GoalY - 8) * Zm, Zm * 2, Zm);
						DrawImage(level.LH.GameStyle | Data::OBJ_27G,
				...
				DrawImage(level.LH.GameStyle | Data::OBJ_38, 1 * Zm,
					(level.MapHdr.BorT / 16 - level.LH.StartY - 3) * Zm, Zm * 3, Zm * 3);
			}
		}

		int Drawer::GetGrdType(const std::string& A) {
			int p = 0;
			int i = 0;
			p     = 0;
			for(i = 0; i <= 7; i++) {
		...
		std::string Drawer::GetGrdCode(int x, int y) {
			std::string S = "";
			// C
		...
		int Drawer::GetGrdBold(int x, int y) {
		...
		int Drawer::CalC(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4) {
			return ((level.GroundNode[x1][y1] == 0) ? 0 : 1000)
				   + ((level.GroundNode[x2][y2] == 0) ? 0 : 100)
				   + ((level.GroundNode[x3][y3] == 0) ? 0 : 10)
				   + ((level.GroundNode[x4][y4] == 0) ? 0 : 1);
		}

		Parser::Point Drawer::GetCorCode(int x, int y) {
		...
		void Drawer::DrawGrdCode() {
			...
			for(i = 1; i <= W + 1; i++) {
				for(j = 1; j <= H + 1; j++) {
					...
					case 0:
						// G.DrawString(GroundNode(i, j), Me.Font, Brushes.Black, (i - 1) * Zm, (H -
						// j) * Zm)
						break;
					case 6:
					case 7:
			...
			for(i = 0; i <= W; i++) {
				for(j = 0; j <= H; j++) {
					if(level.GroundNode[i][j] == 1) {
						...
						DrawTile(R.X, R.Y, 1, 1, (i - 1) * Zm, (H - j) * Zm, Zm, Zm);
						// G.DrawString(GroundNode(i, j), Me.Font, Brushes.Black, (i - 1) * Zm, (H -
						// j) * Zm) G.DrawString(GetCorCode(i, j).X & "," & GetCorCode(i, j).Y,
						// Me.Font, Brushes.Black, (i - 1) * Zm, (H
						// - j) * Zm + 10)
					}
				}
			}
		...
		void Drawer::DrawTrack() {
			for(int i = 0; i < level.MapHdr.TrackCount; i++) {
				...
				uint32_t path = 0;
				if(trackType == 0) {
					path = level.LH.GameStyle | Data::OBJ_T0;
				} else if(trackType == 1) {
					path = level.LH.GameStyle | Data::OBJ_T1;
				} else if(trackType < 10) {
					path = level.LH.GameStyle | (Data::OBJ_T2 + (trackType - 2) * 32768);
				} else {
					path = level.LH.GameStyle | (Data::OBJ_T10 + (trackType - 10) * 32768);
				}

				if(level.MapTrk[i].Type < 8) {
					DrawImage(path, level.MapTrk[i].X * Zm - Zm,
						(H - 2) * Zm - level.MapTrk[i].Y * Zm, Zm * 3, Zm * 3);
					switch(level.MapTrk[i].Type) {
					case 0:
						if(level.TrackNode[level.MapTrk[i].X + 1 + 1][level.MapTrk[i].Y + 1] == 1
							&& level.MapTrk[i].F0 == 0) {
							DrawImage(level.LH.GameStyle | Data::OBJ_T, level.MapTrk[i].X * Zm + Zm,
					...
					case 1:
						if(level.TrackNode[level.MapTrk[i].X + 1][level.MapTrk[i].Y + 1 + 1] == 1
							&& level.MapTrk[i].F0 == 0) {
							DrawImage(level.LH.GameStyle | Data::OBJ_T, level.MapTrk[i].X * Zm,
				...
				{
					DrawImage(path, level.MapTrk[i].X * Zm - Zm,
						(H - 4) * Zm - level.MapTrk[i].Y * Zm, Zm * 5, Zm * 5);

					if(level.TrackNode[level.MapTrk[i].X
									   + level.TrackYPt[level.MapTrk[i].Type][0].X]
									  [level.MapTrk[i].Y
										  + level.TrackYPt[level.MapTrk[i].Type][0].Y]
							== 1
						&& level.MapTrk[i].F0 == 0) {
						DrawImage(level.LH.GameStyle | Data::OBJ_T,
							(level.MapTrk[i].X - 1 + level.TrackYPt[level.MapTrk[i].Type][0].X)
								* Zm,
							(H - 4) * Zm
					...
					if(level.TrackNode[level.MapTrk[i].X
									   + level.TrackYPt[level.MapTrk[i].Type][1].X]
									  [level.MapTrk[i].Y
										  + level.TrackYPt[level.MapTrk[i].Type][1].Y]
							== 1
						&& level.MapTrk[i].F1 == 0) {
						DrawImage(level.LH.GameStyle | Data::OBJ_T,
							(level.MapTrk[i].X - 1 + level.TrackYPt[level.MapTrk[i].Type][1].X)
								* Zm,
							(H - 4) * Zm
					...
					if(level.TrackNode[level.MapTrk[i].X
									   + level.TrackYPt[level.MapTrk[i].Type][2].X]
									  [level.MapTrk[i].Y
										  + level.TrackYPt[level.MapTrk[i].Type][2].Y]
							== 1
						&& level.MapTrk[i].F2 == 0) {
						DrawImage(level.LH.GameStyle | Data::OBJ_T,
							(level.MapTrk[i].X - 1 + level.TrackYPt[level.MapTrk[i].Type][2].X)
								* Zm,
							(H - 4) * Zm
		...
		void Drawer::DrawCID() {
			...
			for(int i = 0; i < level.MapHdr.ObjCount; i++) {
				...
				default:
					if((level.MapObj[i].CFlag / 0x4) % 2 == 1) {
						path = Data::GetIndex(level.LH.GameStyle, objCid, Data::A_, Data::CID);
					} else {
						path = Data::GetIndex(level.LH.GameStyle, objCid, Data::NONE, Data::CID);
					}
			...
			for(int i = 0; i < level.MapHdr.ObjCount; i++) {
				...
				if(level.MapObj[i].ID == 24) {
					...
					for(j = 0; j <= (objFlag - 0x6000000) / 0x400000 + 1; j++) {
						DrawImageRotateOpacity(level.LH.GameStyle | Data::OBJ_24A, FR, 0.5,
							-Zm / 4 + j * std::cos(FR) * Zm + LX + Zm / 2,
							-Zm / 4 + j * std::sin(FR) * Zm + LY + Zm / 2, Zm, Zm / 2);
					}

					if((objFlag / 0x8) % 2 == 1) {
						DrawImage(Data::OBJ_CMN_B0, LX, LY, Zm, Zm);
					} else {
						DrawImage(Data::OBJ_CMN_B1, LX, LY, Zm, Zm);
					}
				}
			}
		}

		void Drawer::DrawFire() {
			for(int i = 0; i < level.MapHdr.ObjCount; i++) {
				int objW = level.MapObj[i].W;
				int objH = level.MapObj[i].H;
				int objX = level.MapObj[i].X;
				...
				if(level.MapObj[i].ID == 54) {
					...
					case 0x40:
						DrawImageOpacity(level.LH.GameStyle | Data::OBJ_54A1, 0.5,
							(float)((-objW / 2.0 + objX / 160.0) * Zm),
							(H - 3) * Zm - (float)((objH - 0.5 + objY / 160.0) * Zm), Zm * objW,
							3 * Zm * objH);
						break;
					case 0x48:
						DrawImageOpacity(level.LH.GameStyle | Data::OBJ_54A3, 0.5,
							(float)((-objW / 2.0 + 1 + objX / 160.0) * Zm),
							H * Zm - (float)((objH - 0.5 + objY / 160.0) * Zm), 3 * Zm * objW,
							Zm * objH);
						break;
					case 0x50:
						DrawImageOpacity(level.LH.GameStyle | Data::OBJ_54A5, 0.5,
							(float)((-objW / 2.0 + objX / 160.0) * Zm),
							(H + 1) * Zm - (float)((objH - 0.5 + objY / 160.0) * Zm), Zm * objW,
							3 * Zm * objH);
						break;
					case 0x58:
						DrawImageOpacity(level.LH.GameStyle | Data::OBJ_54A7, 0.5,
							(float)((-objW / 2.0 - 3 + objX / 160.0) * Zm),
							H * Zm - (float)((objH - 0.5 + objY / 160.0) * Zm), 3 * Zm * objW,
							Zm * objH);
						break;
					case 0x44:
						DrawImageOpacity(level.LH.GameStyle | Data::OBJ_54A2, 0.5,
							(float)((-objW / 2.0 + objX / 160.0) * Zm),
							(H - 3) * Zm - (float)((objH - 0.5 + objY / 160.0) * Zm), Zm * objW,
							3 * Zm * objH);
						break;
					case 0x4C:
						DrawImageOpacity(level.LH.GameStyle | Data::OBJ_54A4, 0.5,
							(float)((-objW / 2.0 + 1 + objX / 160.0) * Zm),
							H * Zm - (float)((objH - 0.5 + objY / 160.0) * Zm), 3 * Zm * objW,
							Zm * objH);
						break;
					case 0x54:
						DrawImageOpacity(level.LH.GameStyle | Data::OBJ_54A6, 0.5,
							(float)((-objW / 2.0 + objX / 160.0) * Zm),
							(H + 1) * Zm - (float)((objH - 0.5 + objY / 160.0) * Zm), Zm * objW,
							3 * Zm * objH);
						break;
					case 0x5C:
						DrawImageOpacity(level.LH.GameStyle | Data::OBJ_54A8, 0.5,
							(float)((-objW / 2.0 - 3 + objX / 160.0) * Zm),
							H * Zm - (float)((objH - 0.5 + objY / 160.0) * Zm), 3 * Zm * objW,
							Zm * objH);
						break;
				...
				for(int J = 0; J < level.MapCPipe[i].NodeCount; J++) {
					if(J == 0) {
						for(K = 0; K < level.MapCPipe[i].Node[J].H; K++) {
							...
							case 0: // R
								if(K == 0) {
									path = level.LH.GameStyle | Data::OBJ_93C;
								} else if(level.MapCPipe[i].NodeCount == 1
										  && K == level.MapCPipe[i].Node[J].H - 1) {
									path = level.LH.GameStyle | Data::OBJ_93E;
								} else {
									path = level.LH.GameStyle | Data::OBJ_93D;
								}
							...
							case 2: // U
								if(K == 0) {
									path = level.LH.GameStyle | Data::OBJ_93;
								} else if(level.MapCPipe[i].NodeCount == 1
										  && K == level.MapCPipe[i].Node[J].H - 1) {
									path = level.LH.GameStyle | Data::OBJ_93B;
								} else {
									path = level.LH.GameStyle | Data::OBJ_93A;
								}
						...
						if(level.MapCPipe[i].Node[J].type >= 3
							&& level.MapCPipe[i].Node[J].type <= 10) {
							switch(level.MapCPipe[i].Node[J].type) {
							case 3:
							case 7: // RU DL
								DrawImage(level.LH.GameStyle | Data::OBJ_93G,
									(level.MapCPipe[i].Node[J].X) * Zm,
									(H - 2.0 - level.MapCPipe[i].Node[J].Y) * Zm, 2 * Zm, 2 * Zm);
								break;
							case 4:
							case 9: // RD UL
								DrawImage(level.LH.GameStyle | Data::OBJ_93H,
									(level.MapCPipe[i].Node[J].X) * Zm,
									(H - 2.0 - level.MapCPipe[i].Node[J].Y) * Zm, 2 * Zm, 2 * Zm);
								break;
							case 6:
							case 10: // UR LD
								DrawImage(level.LH.GameStyle | Data::OBJ_93J,
									(level.MapCPipe[i].Node[J].X) * Zm,
									(H - 2.0 - level.MapCPipe[i].Node[J].Y) * Zm, 2 * Zm, 2 * Zm);
								break;
							case 5:
							case 8: // DR LU
								DrawImage(level.LH.GameStyle | Data::OBJ_93F,
									(level.MapCPipe[i].Node[J].X) * Zm,
									(H - 2.0 - level.MapCPipe[i].Node[J].Y) * Zm, 2 * Zm, 2 * Zm);
								break;
							}
							...
							for(K = 0; K < level.MapCPipe[i].Node[J].H; K++) {
								switch(level.MapCPipe[i].Node[J].Dir) {
								case 0: // R
									DrawImage(level.LH.GameStyle | Data::OBJ_93D,
								...
								case 1: // L
									...
									break;
								case 2: // U
									DrawImage(level.LH.GameStyle | Data::OBJ_93A,
										level.MapCPipe[i].Node[J].X * Zm,
										(H - 1 - level.MapCPipe[i].Node[J].Y - K) * Zm, 2 * Zm, Zm);
									break;
								case 3: // D
