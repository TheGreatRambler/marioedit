...
namespace MM2 {
	...
	void level_t::_read() {
		...
		m_clear_condition_category
			= static_cast<level_t::clear_condition_category_t>(m__io->read_u1());
		...
		m_description
			= kaitai::kstream::bytes_to_str(m__io->read_bytes(202), std::string("UTF-16LE"));
	...
	void level_t::map_t::_read() {
		...
		int l_objects                  = 2600;
		m_objects                      = std::unique_ptr<std::vector<std::unique_ptr<obj_t>>>(
            new std::vector<std::unique_ptr<obj_t>>());
		m_objects->reserve(l_objects);
		for(int i = 0; i < l_objects; i++) {
			m_objects->push_back(
				std::move(std::unique_ptr<obj_t>(new obj_t(m__io, this, m__root))));
		}
		int l_sounds = 300;
		m_sounds     = std::unique_ptr<std::vector<std::unique_ptr<sound_t>>>(
            new std::vector<std::unique_ptr<sound_t>>());
		m_sounds->reserve(l_sounds);
		for(int i = 0; i < l_sounds; i++) {
			m_sounds->push_back(
				std::move(std::unique_ptr<sound_t>(new sound_t(m__io, this, m__root))));
		}
		int l_snakes = 5;
		m_snakes     = std::unique_ptr<std::vector<std::unique_ptr<snake_t>>>(
            new std::vector<std::unique_ptr<snake_t>>());
		m_snakes->reserve(l_snakes);
		for(int i = 0; i < l_snakes; i++) {
			m_snakes->push_back(
				std::move(std::unique_ptr<snake_t>(new snake_t(m__io, this, m__root))));
		}
		...
		for(int i = 0; i < l_clear_pipes; i++) {
			m_clear_pipes->push_back(
				std::move(std::unique_ptr<clear_pipe_t>(new clear_pipe_t(m__io, this, m__root))));
		}
		...
		for(int i = 0; i < l_piranha_creepers; i++) {
			m_piranha_creepers->push_back(std::move(
				std::unique_ptr<piranha_creeper_t>(new piranha_creeper_t(m__io, this, m__root))));
		}
		...
		for(int i = 0; i < l_exclamation_blocks; i++) {
			m_exclamation_blocks->push_back(std::move(std::unique_ptr<exclamation_block_t>(
				new exclamation_block_t(m__io, this, m__root))));
		}
		...
		int l_ground = 4000;
		m_ground     = std::unique_ptr<std::vector<std::unique_ptr<ground_t>>>(
            new std::vector<std::unique_ptr<ground_t>>());
		m_ground->reserve(l_ground);
		for(int i = 0; i < l_ground; i++) {
			m_ground->push_back(
				std::move(std::unique_ptr<ground_t>(new ground_t(m__io, this, m__root))));
		}
		int l_tracks = 1500;
		m_tracks     = std::unique_ptr<std::vector<std::unique_ptr<track_t>>>(
            new std::vector<std::unique_ptr<track_t>>());
		m_tracks->reserve(l_tracks);
		for(int i = 0; i < l_tracks; i++) {
			m_tracks->push_back(
				std::move(std::unique_ptr<track_t>(new track_t(m__io, this, m__root))));
		}
		int l_icicles = 300;
		m_icicles     = std::unique_ptr<std::vector<std::unique_ptr<icicle_t>>>(
            new std::vector<std::unique_ptr<icicle_t>>());
		m_icicles->reserve(l_icicles);
		for(int i = 0; i < l_icicles; i++) {
			m_icicles->push_back(
				std::move(std::unique_ptr<icicle_t>(new icicle_t(m__io, this, m__root))));
		}
	...
	void level_t::snake_t::_read() {
		...
		for(int i = 0; i < l_nodes; i++) {
			m_nodes->push_back(
				std::move(std::unique_ptr<snake_node_t>(new snake_node_t(m__io, this, m__root))));
		}
	}
	...
	void level_t::clear_pipe_t::_read() {
		...
		for(int i = 0; i < l_nodes; i++) {
			m_nodes->push_back(std::move(
				std::unique_ptr<clear_pipe_node_t>(new clear_pipe_node_t(m__io, this, m__root))));
		}
	}
	...
	void level_t::exclamation_block_t::_read() {
		...
		m_nodes->reserve(l_nodes);
		for(int i = 0; i < l_nodes; i++) {
			m_nodes->push_back(std::move(std::unique_ptr<exclamation_block_node_t>(
				new exclamation_block_node_t(m__io, this, m__root))));
		}
	}
	...
	level_t::snake_node_t::snake_node_t(
		kaitai::kstream* p__io, level_t::snake_t* p__parent, level_t* p__root)
		: kaitai::kstruct(p__io) {
		m__parent = p__parent;
		m__root   = p__root;
		_read();
	...
	void level_t::track_block_t::_read() {
		...
		for(int i = 0; i < l_nodes; i++) {
			m_nodes->push_back(std::move(
				std::unique_ptr<track_block_node_t>(new track_block_node_t(m__io, this, m__root))));
		}
	}
	...
	void level_t::piranha_creeper_t::_read() {
		...
		m_nodes->reserve(l_nodes);
		for(int i = 0; i < l_nodes; i++) {
			m_nodes->push_back(std::move(std::unique_ptr<piranha_creeper_node_t>(
				new piranha_creeper_node_t(m__io, this, m__root))));
		}
	}
