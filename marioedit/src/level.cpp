...
const char* ObjEng[] = { "Goomba", "Koopa", "Piranha Flower", "Hammer Bro", "Block", "? Block",
	"Stone", "Hard Block", "Coin", "Pipe", "Spring", "Lift", "Thwomp", "Bullet Bill Blaster",
	"Mushroom Platform", "Bob-omb", "Semisolid Platform", "Bridge", "P Switch", "POW",
	"Super Mushroom", "Donut Block", "Cloud", "Note Block", "Fire Bar", "Spiny", "Hard Block",
	"Goal", "Buzzy Beetle", "Hidden Block", "Lakitu", "Lakitu Cloud", "Banzai Bill", "1UP",
	"Fire Flower", "Super Star", "Lava Lift", "Starting Brick", "Starting Arrow", "Magikoopa",
	"Spike Top", "Boo", "Clown Car", "Spikes", "Big Mushroom", "Shoe Goomba", "Dry Bones", "Cannon",
	"Blooper", "Castle Bridge", "Jumping Machine", "Skipsqueak", "Wiggler", "Conveyor Belt",
	"Burner", "Door", "Cheep Cheep", "Muncher", "Rocky Wrench", "Track", "Lava Bubble",
	"Chain Chomp", "Bowser", "Ice Block", "Vine", "Stingby", "Arrow", "One-Way", "Saw", "Player",
	"Big Coin", "Half Collision Platform", "Koopa Car", "Cinobio (not yet named)", "Spike/Ball",
	"Stone", "Twister", "Boom Boom", "Pokey", "P Block", "Sprint Platform", "SMB2 Mushroom",
	"Donut", "Skewer", "Snake Block", "Spike Block", "Charvaargh", "Slight Slope", "Steep Slope",
	"Reel Camera", "Checkpoint Flag", "Seesaw", "Red Coin", "Clear Pipe", "Conveyor Belt", "Key",
	"Ant Trooper", "Warp Box", "Bowser Jr", "ON/OFF Block", "Dotted-Line Block", "Water Marker",
	"Monty Mole", "Fish Bone", "Angry Sun", "Swinging Claw", "Tree", "Piranha Creeper",
	"Blinking Block", "Sound Effect", "Spike Block", "Mechakoopa", "Crate", "Mushroom Trampoline",
	"Porkupuffer", "Cinobic", "Super Hammer", "Bully", "Icicle", "! Block", "Lemmy", "Morton",
	"Larry", "Wendy", "Iggy", "Roy", "Ludwig", "Cannon Box", "Propeller Box", "Goomba Mask",
	"Bullet Bill Mask", "Red POW Box", "ON/OFF Trampoline" };
...
void LevelDrawer::Setup() {
	...
	static uint8_t GS[] = {  };
...
void LevelDrawer::SetGraphics(SkCanvas* canvas) {
	c = canvas;
}

sk_sp<SkImage> LevelDrawer::ImageFromPath(std::string path) {
	auto bitmap = new SkBitmap();
	auto codec  = SkCodec::MakeFromStream(SkStream::MakeFromFile(path.c_str()));

	if(!codec) {
		return NULL;
	}

	SkImageInfo info = codec->getInfo().makeColorType(kBGRA_8888_SkColorType);
...
void LevelDrawer::LoadTilesheet() {
	tilesheetPath = fmt::format("{}/img/tile/{}-{}{}.png", assetFolder, level.LH.GameStyle,
		level.MapHdr.Theme, (level.MapHdr.Flag == 2) ? "A" : "");

	tilesheet = ImageFromPath(tilesheetPath);
	if(!tilesheet) {
		fmt::print("Tilesheet {} does not exist\n", tilesheetPath);
	}
	TileW = tilesheet->width() / 16;

	std::string spritesheetPath = fmt::format("{}/img/spritesheet.png", assetFolder);
	spritesheet                 = ImageFromPath(spritesheetPath);
	if(!spritesheet) {
...
void LevelDrawer::SetOffsetX(int x) {
	offsetX = x;
}

void LevelDrawer::SetOffsetY(int y) {
	offsetY = y;
}
...
void LevelDrawer::DrawGridlines() {
	...
	SkPaint lavaPaint;
	lavaPaint.setColor(SkColorSetARGB(0.4 * 255, 1.0 * 255, 0.0 * 255, 0.0 * 255));
	SkPaint waterPaint;
	waterPaint.setColor(SkColorSetARGB(0.4 * 255, 0.0 * 255, 0.0 * 255, 1.0 * 255));

	for(i = 0; i <= H; i++) {
		linePaint.setColor(SkColorSetARGB(0.25 * 255, 0.41 * 255, 0.41 * 255, 0.41 * 255));
		c->drawLine(SkPoint::Make(0 + offsetX, i * Zm + offsetY),
			SkPoint::Make(W * Zm + offsetX, i * Zm + offsetY), linePaint);

		if(i % 13 == 0) {
			c->drawLine(SkPoint::Make(0 + offsetX, (H - i) * Zm + 1 + offsetY),
				SkPoint::Make(W * Zm + offsetX, (H - i) * Zm + 1 + offsetY), linePaint);
		}

		if((H - i) % 10 == 0) {
			auto str = std::to_string(H - i).c_str();
			c->drawSimpleText(str, strlen(str), SkTextEncoding::kUTF8, 3 + offsetX,
				(i - 0.25) * Zm + offsetY, textFont, textPaint);
		}
	}

	for(i = 0; i <= W; i++) {
		linePaint.setColor(SkColorSetARGB(0.25 * 255, 0.41 * 255, 0.41 * 255, 0.41 * 255));
		c->drawLine(SkPoint::Make(i * Zm + offsetX, 0 + offsetY),
			SkPoint::Make(i * Zm + offsetX, H * Zm + offsetY), linePaint);

		if(i % 24 == 0) {
			c->drawLine(SkPoint::Make(i * Zm + 1 + offsetX, 0 + offsetY),
				SkPoint::Make(i * Zm + 1 + offsetX, H * Zm + offsetY), linePaint);
		}

		if(i % 10 == 9) {
			auto str = std::to_string(i + 1).c_str();
			c->drawSimpleText(str, strlen(str), SkTextEncoding::kUTF8, i * Zm + 3 + offsetX,
				Zm * 0.75 + offsetY, textFont, textPaint);
		}
	}

	c->drawRect(SkRect::MakeXYWH(0 + offsetX, (H - level.MapHdr.LiqEHeight - 0.5) * Zm + offsetY,
					W * Zm, H * Zm),
		level.MapHdr.Theme == 2 ? lavaPaint : waterPaint);
	c->drawRect(SkRect::MakeXYWH(0 + offsetX, (H - level.MapHdr.LiqSHeight - 0.5) * Zm + offsetY,
					W * Zm, H * Zm),
		level.MapHdr.Theme == 2 ? lavaPaint : waterPaint);
}

sk_sp<SkImage> LevelDrawer::GetCachedTile(int x, int y, int w, int h) {
	int index = (y * 16 + x) * 3;
	...
	if(tileCache.count(index)) {
		...
		rasterSurface->getCanvas()->drawImageRect(tilesheet,
			...
			SkRect::MakeXYWH(0, 0, TileW * w, TileW * h), SkSamplingOptions(SkFilterMode::kNearest),
			nullptr, SkCanvas::kStrict_SrcRectConstraint);
		auto image       = rasterSurface->makeImageSnapshot();
		tileCache[index] = image;
		return image;
	}
}

sk_sp<SkImage> LevelDrawer::GetCachedPattern(int id) {
	if(patternCache.count(id)) {
		return patternCache[id];
	} else {
		const auto sprite  = LevelData::ObjectLocation.at(id);
		auto rasterSurface = SkSurface::MakeRasterN32Premul(sprite.width, sprite.height);
		rasterSurface->getCanvas()->drawImageRect(spritesheet,
			SkRect::MakeXYWH(sprite.x, sprite.y, sprite.width, sprite.height),
			SkRect::MakeXYWH(0, 0, sprite.width, sprite.height),
			SkSamplingOptions(SkFilterMode::kNearest), nullptr,
			SkCanvas::kStrict_SrcRectConstraint);
		auto image       = rasterSurface->makeImageSnapshot();
		patternCache[id] = image;
		return image;
	}
}
...
void LevelDrawer::DrawTile(
	...
	auto pattern = GetCachedTile(tileX, tileY, tileW, tileH);
	c->drawImageRect(pattern, SkRect::MakeXYWH(0, 0, pattern->width(), pattern->height()),
		SkRect::MakeXYWH(x + offsetX, y + offsetY, targetWidth, targetHeight),
...
void LevelDrawer::DrawImage(uint32_t id, int x, int y, int targetWidth, int targetHeight) {
	if(doLogging) {
		fmt::print(
			"Printing {} at X:{} Y:{} with W:{} and H:{}\n", id, x, y, targetWidth, targetHeight);
	}

	auto pattern = GetCachedPattern(id);
	if(pattern) {
		const auto sprite = LevelData::ObjectLocation.at(id);
		c->drawImageRect(pattern, SkRect::MakeXYWH(0, 0, pattern->width(), pattern->height()),
			SkRect::MakeXYWH(x + offsetX, y + offsetY, targetWidth, targetHeight),
...
void LevelDrawer::DrawImageOpacity(
	...
	if(pattern) {
		const auto sprite = LevelData::ObjectLocation.at(id);
		auto paint        = SkPaint(SkColor4f::FromColor(SkColorSetARGB(opacity * 255, 0, 0, 0)));
		c->drawImageRect(pattern, SkRect::MakeXYWH(0, 0, pattern->width(), pattern->height()),
			SkRect::MakeXYWH(x + offsetX, y + offsetY, targetWidth, targetHeight),
			SkSamplingOptions(SkFilterMode::kNearest), &paint, SkCanvas::kStrict_SrcRectConstraint);
	}
}

void LevelDrawer::DrawImageRotate(
			...
			if(objID == 105) {
				...
				if(objCid != -1) {
					path = LevelData::GetIndex(level.LH.GameStyle, objCid,
						(level.MapObj[i].CFlag / 0x4) % 2 == 1 ? LevelData::A_ : LevelData::NONE,
						LevelData::CID);
				...
				if((objLid + 1 == 0 && !L) || (objLid + 1 > 0 && L) || objID == 9) {
					...
					case 132: {
						if((objFlag / 0x4) % 2 == 1) {
							for(int j = 0; j < objW; j++) {
								if(j == 0) {
									path = LevelData::GetIndex(
										level.LH.GameStyle, objID, LevelData::D_);
								} else if(j == objW - 1) {
									path = LevelData::GetIndex(
										level.LH.GameStyle, objID, LevelData::E_);
								} else {
									path = LevelData::GetIndex(
										level.LH.GameStyle, objID, LevelData::C_);
								}
							...
							for(int j = 0; j < objW; j++) {
								if(j == 0) {
									path = LevelData::GetIndex(
										level.LH.GameStyle, objID, LevelData::A_);
								} else if(j == objW - 1) {
									path = LevelData::GetIndex(
										level.LH.GameStyle, objID, LevelData::B_);
								} else {
					...
					case 106: //树
					{
						...
						DrawImage(level.LH.GameStyle | LevelData::OBJ_106,
							(float)((-objW / 2.0 + objX / 160.0) * Zm),
					...
					case 53: {
						...
						for(int j = 0; j < objW; j++) {
							...
							if((objFlag) / 0x40000 % 2 == 0) {
								if((objFlag / 0x8) % 2 == 1) {
									DrawImage(LevelData::OBJ_CMN_A1,
										LX + std::round((-0.5 + objW / 2) * Zm), LY, Zm, Zm);
								} else {
									DrawImage(LevelData::OBJ_CMN_A0,
										LX + std::round((-0.5 + objW / 2) * Zm), LY, Zm, Zm);
								}
							} else {
								if((objFlag / 0x8) % 2 == 1) {
									DrawImage(LevelData::OBJ_CMN_A3,
										LX + std::round((-0.5 + objW / 2) * Zm), LY, Zm, Zm);
								} else {
									DrawImage(LevelData::OBJ_CMN_A2,
										LX + std::round((-0.5 + objW / 2) * Zm), LY, Zm, Zm);
								}
							}
						}
					...
					case 9: {
						...
						if(cmnId != -1) {
							path = LevelData::GetIndex(
								0, LevelData::C0 + cmnId, LevelData::NONE, LevelData::CMN);
						}
					...
					case 86: {
						path = LevelData::GetIndex(level.LH.GameStyle, objID,
							(objFlag / 0x4) % 2 == 1 ? LevelData::A_ : LevelData::NONE);
					...
					case 40: {
						...
						if((objFlag / 0x4) % 2 == 1) {
							switch((objFlag / 0x1000000) % 8) {
								//方向6上 4下 0左 2右
							case 0x0: // L
								DrawImage(level.LH.GameStyle | LevelData::OBJ_40B0, LX, LY,
									Zm * objW, Zm * objH);
								break;
							case 0x2: // R
								DrawImage(level.LH.GameStyle | LevelData::OBJ_40B2, LX, LY,
									Zm * objW, Zm * objH);
								break;
							case 0x4: // D
								DrawImage(level.LH.GameStyle | LevelData::OBJ_40B4, LX, LY,
									Zm * objW, Zm * objH);
								break;
							case 0x6: // U
								DrawImage(level.LH.GameStyle | LevelData::OBJ_40B6, LX, LY,
									Zm * objW, Zm * objH);
								break;
							}
						} else {
							switch((objFlag / 0x1000000) % 8) {
								//方向6上 4下 0左 2右
							case 0x0: // L
								DrawImage(level.LH.GameStyle | LevelData::OBJ_40A0, LX, LY,
									Zm * objW, Zm * objH);
								break;
							case 0x2: // R
								DrawImage(level.LH.GameStyle | LevelData::OBJ_40A2, LX, LY,
									Zm * objW, Zm * objH);
								break;
							case 0x4: // D
								DrawImage(level.LH.GameStyle | LevelData::OBJ_40A0, LX, LY,
								...
								break;
							case 0x6: // U
								DrawImage(level.LH.GameStyle | LevelData::OBJ_40A6, LX, LY,
									Zm * objW, Zm * objH);
								break;
							}
						}
					...
					case 30: {
						...
						DrawImage(level.LH.GameStyle | LevelData::OBJ_31, LX - Zm / 2, LY + Zm / 2,
							Zm * 2, Zm);
						break;
					}
					case 31: {
					...
					case 13: {
						...
						for(int j = 2; j < objH; j++) {
							if((objFlag / 0x4) % 2 == 1) {
								DrawImageOpacity(level.LH.GameStyle | LevelData::OBJ_13C, 0.7, LX,
									LY + j * Zm, Zm, Zm);
							} else {
								DrawImageOpacity(level.LH.GameStyle | LevelData::OBJ_13A, 0.7, LX,
									LY + j * Zm, Zm, Zm);
							}
						}
					...
					case 39: {
						...
						DrawImage(level.LH.GameStyle | LevelData::OBJ_39, LX - Zm - Zm,
					...
					case 61: {
						...
						if((objFlag / 0x4) % 2 == 0) {
							DrawImage(level.LH.GameStyle | LevelData::OBJ_61A,
								(float)((-0.5 + objX / 160.0) * Zm),
								H * Zm - (float)((0.5 + objY / 160.0) * Zm) + KY, Zm, Zm);
						}
						DrawImage(level.LH.GameStyle | LevelData::OBJ_61,
							(float)((-objW / 2.0 + objX / 160.0) * Zm),
							H * Zm - (float)((objH - 0.5 + objY / 160.0) * Zm) + KY, Zm * objW,
							Zm * objH);
						break;
					...
					case 98: {
						...
						DrawImage(level.LH.GameStyle | LevelData::OBJ_98,
					...
					case 103: {
						...
						DrawImage(level.LH.GameStyle | LevelData::OBJ_103,
					...
					case 91: {
						...
						DrawImage(level.LH.GameStyle | LevelData::OBJ_91C,
							(float)((-0.5 + objX / 160.0) * Zm),
							H * Zm - (float)((objH - 0.5 + objY / 160.0) * Zm), Zm, Zm);
					...
					case 105: {
						...
						if((objFlag / 0x80) % 2 == 1) {
							DrawImage(
								level.LH.GameStyle | LevelData::OBJ_105A, LX, LY, Zm * 3, Zm * 5);
						} else {
							DrawImage(
								level.LH.GameStyle | LevelData::OBJ_105, LX, LY, Zm * 3, Zm * 5);
						}
						break;
					}
					}
					...
					if(L && level.ObjLinkType[objLid + 1] == 59) {
						...
						path      = LevelData::GetIndex(
								 0, LevelData::D0 + cmnId, LevelData::NONE, LevelData::CMN);
...
void LevelDrawer::ReGrdCode() {
	...
	// 10  11  12缓大左上 13缓大右上 14缓大左下 15缓大右下  16缓小左上 17缓小右上 18缓小左下
	// 19缓小右下 20端左上 21端右上 22端左下 23端右下 24    25    26
	for(m = 0; m <= 2; m++) {
		for(i = 0; i <= W; i++) {
			for(j = 0; j <= H; j++) {
	...
	if(NowIO) {
		...
		case 12621: // 1
			if(level.MapHdr.Theme == 2) {
				DrawImage(level.LH.GameStyle | LevelData::OBJ_27A,
				...
				DrawImage(level.LH.GameStyle | LevelData::OBJ_27,
					(float)((level.LH.GoalX / 10.0 - 0.5) * Zm),
		...
		case 22349: // W
			if(level.MapHdr.Theme == 2) {
				...
				// For i = 1 To 8
				//	G. DrawImage(level.LH.GameStyle |Image.FromFile(PT & "\img\" &
				// LH.GameStyle.ToString & "\obj\27C ),
				// CSng((LH.GoalX /
				// 10 - 0.5) * Zm), (MapHdr.BorT \ 16 - LH.GoalY - i) * Zm, Zm, Zm) 	G.
				// DrawImage(level.LH.GameStyle |Image.FromFile(PT &
				//"\img\" & LH.GameStyle.ToString & "\obj\27E ), CSng((LH.GoalX / 10 + 1.5) * Zm),
				//(MapHdr.BorT \ 16
				//- LH.GoalY - i) * Zm, Zm, Zm) Next G. DrawImage(level.LH.GameStyle
				//|Image.FromFile(PT & "\img\" &
				// LH.GameStyle.ToString &
				// "\obj\27B ), CSng((LH.GoalX / 10 - 0.5) * Zm), (MapHdr.BorT \ 16 - LH.GoalY - 9)
				// * Zm, Zm, Zm) G. DrawImage(level.LH.GameStyle |Image.FromFile(PT & "\img\" &
				// LH.GameStyle.ToString & "\obj\27D ), CSng((LH.GoalX / 10 + 1.5) * Zm),
				// (MapHdr.BorT \ 16 - LH.GoalY - 9) * Zm, Zm, Zm)

				DrawImage(level.LH.GameStyle | LevelData::OBJ_27F,
					(float)((level.LH.GoalX / 10.0 - 0.5) * Zm),
					(float)(level.MapHdr.BorT / 16 - level.LH.GoalY - 8.5) * Zm, Zm, Zm * 9);
				DrawImage(level.LH.GameStyle | LevelData::OBJ_27,
					(float)((level.LH.GoalX / 10.0) * Zm),
					(float)(level.MapHdr.BorT / 16 - level.LH.GoalY - 8) * Zm, Zm * 2, Zm);
				DrawImage(level.LH.GameStyle | LevelData::OBJ_27G,
		...
		DrawImage(level.LH.GameStyle | LevelData::OBJ_38, 1 * Zm,
...
int LevelDrawer::CalC(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4) {
	return ((level.GroundNode[x1][y1] == 0) ? 0 : 1000)
		   + ((level.GroundNode[x2][y2] == 0) ? 0 : 100)
...
void LevelDrawer::DrawGrdCode() {
	...
	for(i = 1; i <= W + 1; i++) {
		for(j = 1; j <= H + 1; j++) {
			...
			case 0:
				// G.DrawString(GroundNode(i, j), Me.Font, Brushes.Black, (i - 1) * Zm, (H - j) *
				// Zm)
				break;
			case 6:
			case 7:
	...
	for(i = 0; i <= W; i++) {
		for(j = 0; j <= H; j++) {
			if(level.GroundNode[i][j] == 1) {
				...
				DrawTile(R.X, R.Y, 1, 1, (i - 1) * Zm, (H - j) * Zm, Zm, Zm);
				// G.DrawString(GroundNode(i, j), Me.Font, Brushes.Black, (i - 1) * Zm, (H - j) *
				// Zm) G.DrawString(GetCorCode(i, j).X & "," & GetCorCode(i, j).Y, Me.Font,
				// Brushes.Black, (i - 1) * Zm, (H
				// - j) * Zm + 10)
			}
		}
	}
...
void LevelDrawer::DrawTrack() {
	for(int i = 0; i < level.MapHdr.TrackCount; i++) {
		...
		if(level.MapTrk[i].Type < 8) {
			DrawImage(path, level.MapTrk[i].X * Zm - Zm, (H - 2) * Zm - level.MapTrk[i].Y * Zm,
				Zm * 3, Zm * 3);
			switch(level.MapTrk[i].Type) {
			case 0:
				if(level.TrackNode[level.MapTrk[i].X + 1 + 1][level.MapTrk[i].Y + 1] == 1
					&& level.MapTrk[i].F0 == 0) {
				...
				break;
			case 1:
				if(level.TrackNode[level.MapTrk[i].X + 1][level.MapTrk[i].Y + 1 + 1] == 1
					&& level.MapTrk[i].F0 == 0) {
		...
		{
			DrawImage(path, level.MapTrk[i].X * Zm - Zm, (H - 4) * Zm - level.MapTrk[i].Y * Zm,
				Zm * 5, Zm * 5);
			...
			if(level.TrackNode[level.MapTrk[i].X + level.TrackYPt[level.MapTrk[i].Type][0].X]
				...
				DrawImage(level.LH.GameStyle | LevelData::OBJ_T,
					...
					(H - 4) * Zm
						- (level.MapTrk[i].Y - level.TrackYPt[level.MapTrk[i].Type][0].Y) * Zm,
					Zm, Zm);
			}
			if(level.TrackNode[level.MapTrk[i].X + level.TrackYPt[level.MapTrk[i].Type][1].X]
				...
				DrawImage(level.LH.GameStyle | LevelData::OBJ_T,
					...
					(H - 4) * Zm
						- (level.MapTrk[i].Y - level.TrackYPt[level.MapTrk[i].Type][1].Y) * Zm,
					Zm, Zm);
			}
			if(level.TrackNode[level.MapTrk[i].X + level.TrackYPt[level.MapTrk[i].Type][2].X]
				...
				DrawImage(level.LH.GameStyle | LevelData::OBJ_T,
					...
					(H - 4) * Zm
						- (level.MapTrk[i].Y - level.TrackYPt[level.MapTrk[i].Type][2].Y) * Zm,
					Zm, Zm);
			}
		}
	}
...
void LevelDrawer::DrawCID() {
	...
	for(int i = 0; i < level.MapHdr.ObjCount; i++) {
		...
		case 116: //状态
			path = LevelData::GetIndex(level.LH.GameStyle, objCid,
				(level.MapObj[i].CFlag / 0x40000) % 2 == 1 ? LevelData::A_ : LevelData::NONE,
				LevelData::CID);
			DrawImage(path, LX, LY, Zm, Zm);
		...
		default:
			if((level.MapObj[i].CFlag / 0x4) % 2 == 1) {
				path = LevelData::GetIndex(
					level.LH.GameStyle, objCid, LevelData::A_, LevelData::CID);
			} else {
				path = LevelData::GetIndex(
					level.LH.GameStyle, objCid, LevelData::NONE, LevelData::CID);
			}
		...
		if(level.MapObj[i].ID == 24) {
			...
			for(j = 0; j <= (objFlag - 0x6000000) / 0x400000 + 1; j++) {
				DrawImageRotateOpacity(level.LH.GameStyle | LevelData::OBJ_24A, FR, 0.5,
					-Zm / 4 + j * std::cos(FR) * Zm + LX + Zm / 2,
					-Zm / 4 + j * std::sin(FR) * Zm + LY + Zm / 2, Zm, Zm / 2);
			}
			...
			if(J == 0) {
				for(K = 0; K < level.MapCPipe[i].Node[J].H; K++) {
					...
					case 0: // R
						...
						} else if(level.MapCPipe[i].NodeCount == 1
								  && K == level.MapCPipe[i].Node[J].H - 1) {
				...
				if(level.MapCPipe[i].Node[J].type >= 3 && level.MapCPipe[i].Node[J].type <= 10) {
					...
					case 3:
					case 7: // RU DL
						DrawImage(level.LH.GameStyle | LevelData::OBJ_93G,
							(level.MapCPipe[i].Node[J].X) * Zm,
							(H - 2.0 - level.MapCPipe[i].Node[J].Y) * Zm, 2 * Zm, 2 * Zm);
						break;
					case 4:
					case 9: // RD UL
						DrawImage(level.LH.GameStyle | LevelData::OBJ_93H,
							(level.MapCPipe[i].Node[J].X) * Zm,
							(H - 2.0 - level.MapCPipe[i].Node[J].Y) * Zm, 2 * Zm, 2 * Zm);
						break;
					case 6:
					case 10: // UR LD
						DrawImage(level.LH.GameStyle | LevelData::OBJ_93J,
							(level.MapCPipe[i].Node[J].X) * Zm,
							(H - 2.0 - level.MapCPipe[i].Node[J].Y) * Zm, 2 * Zm, 2 * Zm);
						break;
					case 5:
					case 8: // DR LU
						DrawImage(level.LH.GameStyle | LevelData::OBJ_93F,
							(level.MapCPipe[i].Node[J].X) * Zm,
							(H - 2.0 - level.MapCPipe[i].Node[J].Y) * Zm, 2 * Zm, 2 * Zm);
						break;
					}
					...
					for(K = 0; K < level.MapCPipe[i].Node[J].H; K++) {
						switch(level.MapCPipe[i].Node[J].Dir) {
						case 0: // R
							DrawImage(level.LH.GameStyle | LevelData::OBJ_93D,
								(level.MapCPipe[i].Node[J].X + K) * Zm,
								(H - 1 - level.MapCPipe[i].Node[J].Y) * Zm, Zm, 2 * Zm);
							break;
						case 1: // L
							DrawImage(level.LH.GameStyle | LevelData::OBJ_93D,
							...
							break;
						case 2: // U
							DrawImage(level.LH.GameStyle | LevelData::OBJ_93A,
								level.MapCPipe[i].Node[J].X * Zm,
								(H - 1 - level.MapCPipe[i].Node[J].Y - K) * Zm, 2 * Zm, Zm);
							break;
						case 3: // D
							DrawImage(level.LH.GameStyle | LevelData::OBJ_93A,
								(level.MapCPipe[i].Node[J].X - 1) * Zm,
								(H - 1 - level.MapCPipe[i].Node[J].Y + K) * Zm, 2 * Zm, Zm);
							break;
						}
					}
